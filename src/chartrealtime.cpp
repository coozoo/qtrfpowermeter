...
//    connect(resetButton,SIGNAL(clicked()),this,SLOT(reset()));
    ...
    // tracer = new QCPItemTracer(customPlot); // parent is enough
    // tracer->setStyle(QCPItemTracer::tsCircle);
    // tracer->setPen(QPen(Qt::darkMagenta, 2)); // border color/thickness
    // tracer->setBrush(Qt::yellow);             // fill color
    // tracer->setSize(12);                      // dot size
    // tracer->setVisible(false);                // start hidden
...
void chartRealTime::reset()
{
    ...
    customPlot->yAxis->setRange(-60, 5);
    ...
    foreach (QString str, graphs)
        {
            ...
            customPlot->graph(i)->setLineStyle(QCPGraph::lsImpulse);
